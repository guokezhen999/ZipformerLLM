...
for ckpt_name in "${checkpoints[@]}"; do
    ...
    for num_chunk in "${num_chunks[@]}"; do
        ...
        for dataset in "${!INPUT_FILES[@]}"; do
            ...
            OUTPUT_FILE=$DECODE_DIR/decode_en_${dataset}_asr_${tag}_chunk_${TOTAL_CHUNK}_punc0.jsonl
            WER_FILE=$DECODE_DIR/wer_en_${dataset}_${tag}_chunk_${TOTAL_CHUNK}_punc0.txt
