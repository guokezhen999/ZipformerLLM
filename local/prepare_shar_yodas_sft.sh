#!/bin/bash
# 将 granary_yodas SFT 两个 JSONL 打成 Lhotse Shar，输出到:
#   data/fbank/speechllm_yodas/shar/{en_asr,en2zh_ast}
#
# 用法（在项目根目录执行）:
#   bash local/prepare_shar_yodas_sft.sh
#   bash local/prepare_shar_yodas_sft.sh --shard-size 10000 --num-jobs 16
#   bash local/prepare_shar_yodas_sft.sh 2>&1 | tee logs/prepare_shar_yodas_sft.log
...
while [[ $# -gt 0 ]]; do
  case "$1" in
    ...
    -h|--help)
      echo "Usage: bash local/prepare_shar_yodas_sft.sh [--shard-size N] [--num-jobs N] [--no-backup]"
      exit 0 ;;
    *)
...
jsonl_inputs=(
  "${CUTS_DIR}/granary_yodas_en_sft_train_selected_5000h.jsonl"
  "${CUTS_DIR}/granary_yodas_en2zh_sft_train_selected_5000h.jsonl"
)
...
log "  python local/generate_shard_duration.py --shar-dir ${SHAR_ROOT}/en_asr --output ${SHAR_ROOT}/en_asr_shard_durations.json"
log "  python local/assign_shards_to_ranks.py --duration-file ${SHAR_ROOT}/en_asr_shard_durations.json --world-size 8 --output exp/shard_assignment_ws8.json"
log "=========================================="
