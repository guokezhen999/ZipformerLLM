...
class AttributeDict(dict):
    ...
    def __str__(self, indent: int = 2):
        def _default_serializer(obj):
            ...
            if isinstance(obj, (pathlib.Path, torch.device)):
                return str(obj)
        ...
        for k, v in self.items():
            try:
                ...
                # If direct serialization fails, apply our custom serializer
                tmp[k] = _default_serializer(v)
        
        return json.dumps(tmp, indent=indent, sort_keys=True, default=_default_serializer)
...
def remove_punctuation_symbol(words):
    """
    Remove both English and Chinese punctuation from a list of characters.

    Args:
        words (list of str): List of single-character strings.

    Returns:
        list of str: Filtered list without punctuation symbols.
    """
    import string 
    # English punctuation
    ...
    table = str.maketrans('', '', ''.join(all_punct))
    return [w.upper().translate(table) for w in words]
    # Filter characters
    # return [ch.upper() for ch in words if ch not in all_punct]
...
def write_error_stats(
    ...
    if remove_punctuation:
        for i, res in enumerate(results):
            cut_id, ref, hyp = res
            ref = remove_punctuation_symbol(ref)
            hyp = remove_punctuation_symbol(hyp)
            results[i] = (cut_id, ref, hyp)
    ...
    for cut_id, ref, hyp in results:
        ...
        for ref_word, hyp_word in ali:
            if True:
                if ref_word == ERR and (not hyp_word or hyp_word.strip() == ""):
                    continue
                if hyp_word == ERR and (not ref_word or ref_word.strip() == ""):
                    continue
    ...
    for cut_id, ref, hyp in results:
        ...
        if True:
            def valid_pair(r, h):
                if r == ERR and (not h or h.strip() == ""):
                    return False
                if h == ERR and (not r or r.strip() == ""):
                    return False
                return True
            filtered_ali = [pair for pair in ali if valid_pair(pair[0], pair[1])]
            ali = filtered_ali
